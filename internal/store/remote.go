...
// ~/.inx/remote/. Only name derivation lives here (this package is the
...
// derived in internal/config/paths.go, which owns INX_HOME resolution.

// RemoteDirName is the directory under the remote ~/.inx that holds all
// remote-module state, and under the local Inx home that holds the
// managed known_hosts file.
const RemoteDirName = "remote"

// RemoteBinDirName holds an uploaded inx binary on the remote host:
// ~/.inx/remote/bin/inx.
