...
func TestPendingUpdateRejectsTargetOutsideGuardInstall(t *testing.T) {
	...
	backup := filepath.Join(home, "repair", "updates", "inx-desktop.previous")
	...
	repairExecutable = func() (string, error) { return filepath.Join(guardDir, "inx-guard"), nil }
...
func TestPendingUpdateRejectsUnexpectedReleaseFile(t *testing.T) {
	...
	bad := []UpdateTransactionFile{
		...
		{TargetPath: filepath.Join(t.TempDir(), "inx-guard"), BackupPath: backup, SHA256: hash},
		{TargetPath: filepath.Join(dir, "inx-guard"), BackupPath: filepath.Join(t.TempDir(), "loose.previous"), SHA256: hash},
		{TargetPath: filepath.Join(dir, "inx-guard"), BackupPath: backup}, // missing hash
		{TargetPath: filepath.Join(dir, "inx-guard"), BackupPath: backup, SHA256: hash, MissingBefore: true},
...
func TestPendingUpdateRejectsBackupSymlinkEscape(t *testing.T) {
	...
	backup := filepath.Join(repairDir, "updates", "inx-desktop.previous")
	if err := os.WriteFile(backup, []byte("old"), 0o700); err != nil {
		t.Fatal(err)
	}
	tx := &UpdateTransaction{
...
func TestPendingUpdateAcceptsMissingReleaseSibling(t *testing.T) {
	...
	tx := &UpdateTransaction{
		...
		Files: []UpdateTransactionFile{
			...
			{TargetPath: filepath.Join(dir, "Inx.exe"), MissingBefore: true},
		},
		CreatedAt: time.Now().UTC().Format(time.RFC3339Nano),
	}
	if err := overwritePendingUpdateForTest(tx); err != nil {
...
func TestPendingUpdateAcceptsWindowsReleaseUnit(t *testing.T) {
	...
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(dir, "inx-launcher.exe"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
...
func TestPendingUpdateAcceptsLinuxReleaseUnit(t *testing.T) {
	...
	names := []string{"inx-desktop", "inx-guard", "inx"}
	paths := make([]string, 0, len(names))
	for _, name := range names {
		path := filepath.Join(dir, name)
		if err := os.WriteFile(path, []byte(name), 0o700); err != nil {
...
func TestPendingUpdateRejectsHashlessOrPrimaryLessTransactions(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(dir, "inx-guard"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	guardBackup := filepath.Join(home, "repair", "updates", "inx-guard.previous")
	txs := map[string]*UpdateTransaction{
		...
		"release unit omits primary executable": {
			...
			Files:    []UpdateTransactionFile{{TargetPath: filepath.Join(dir, "inx-guard"), BackupPath: guardBackup, SHA256: "deadbeef"}},
			Platform: runtime.GOOS + "/" + runtime.GOARCH, CreatedAt: time.Now().UTC().Format(time.RFC3339Nano),
		},
	}
...
func TestPendingUpdateRejectsPortableAliasAsPrimaryTarget(t *testing.T) {
	...
	t.Setenv("INX_HOME", home)
	dir := t.TempDir()
	target := filepath.Join(dir, "Inx.exe")
	guard := filepath.Join(dir, "inx-guard.exe")
	backup := filepath.Join(home, "repair", "updates", "Inx.exe.previous")
	if err := os.MkdirAll(filepath.Dir(backup), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ReadPendingUpdate(); err == nil ||
		!strings.Contains(err.Error(), "not a Inx executable") {
