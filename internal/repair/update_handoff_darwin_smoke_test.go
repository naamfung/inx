...
// Run with:
//
//	INX_MACOS_APPLICATIONS_SMOKE=1 go test ./internal/repair \
...
func TestDarwinApplicationsOrphanBackupLifecycleSmoke(t *testing.T) {
	if os.Getenv("INX_MACOS_APPLICATIONS_SMOKE") != "1" {
		t.Skip("set INX_MACOS_APPLICATIONS_SMOKE=1 to probe /Applications")
	}

	smokeRoot, err := os.MkdirTemp("/Applications", ".inx-updater-smoke-")
	if err != nil {
	...
	app := filepath.Join(smokeRoot, "Inx.app")
	executable := filepath.Join(app, "Contents", "MacOS", "Inx")
