...
// buildExamplePlugin compiles cmd/inx-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "inx-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "inx/cmd/inx-plugin-example").CombinedOutput()
	if err != nil {
