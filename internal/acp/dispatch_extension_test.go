...
func TestUpdateSinkExtensionSupportedClientGetsMetaAndText(t *testing.T) {
	...
	vendor, _ := meta["inx.io"].(map[string]any)
	surface, _ := vendor["extensionSurface"].(map[string]any)
	if surface == nil {
		t.Fatalf("structured update missing _meta.inx.io.extensionSurface: %v", u)
	}
...
func TestInitializeAdvertisesExtensionSurface(t *testing.T) {
	...
	vendor, ok := ir.AgentCapabilities.Meta["inx.io"].(InxExtensionCapabilities)
	if !ok {
		t.Fatalf("_meta[inx.io] = %T", ir.AgentCapabilities.Meta["inx.io"])
	}
	if vendor.ExtensionSurface == nil || !vendor.ExtensionSurface.Supported ||
		vendor.ExtensionSurface.SchemaVersion != inxExtensionSurfaceSchemaVersion {
	...
	got := decoded.AgentCapabilities.Meta["inx.io"].ExtensionSurface
	if got == nil || !got.Supported || got.SchemaVersion != inxExtensionSurfaceSchemaVersion {
...
func TestClientExtensionSurfaceSupportedParsing(t *testing.T) {
	tests := []struct {
		...
		{"capability absent", map[string]any{"inx.io": map[string]any{}}, false},
		{"supported", map[string]any{"inx.io": map[string]any{
			"extensionSurface": map[string]any{"supported": true, "schemaVersion": 1},
		}}, true},
		{"explicit false", map[string]any{"inx.io": map[string]any{
			"extensionSurface": map[string]any{"supported": false},
		}}, false},
		{"malformed vendor", map[string]any{"inx.io": "nope"}, false},
		{"malformed capability", map[string]any{"inx.io": map[string]any{
			"extensionSurface": "nope",
		}}, false},
		{"malformed flag", map[string]any{"inx.io": map[string]any{
