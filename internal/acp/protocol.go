...
// capability blocks (e.g. _meta["inx.io"]) for tolerant parse — unknown
...
// for vendor methods, so the bare "inx/session/reloadExtensions" form
// could collide with a future official ACP method and must not be used.
const sessionReloadExtensionsMethod = "_inx.io/session/reloadExtensions"
...
// inx.extensionSurface in initialize. ACP has no standard notification for
// extension surfaces, so the DTO (the shared eventwire JSON contract) rides
// _meta["inx.io"]["extensionSurface"], mirroring how the initialize
// handshake namespaces vendor data under "inx.io". The sink always pairs
...
// the overrides Inx owns (typically TMPDIR/TMP/TEMP) are sent — never a
