...
func (s *Server) refreshProviderSetup(ref string) {
	...
	if lockErr != nil {
		next.Error = "Unable to inspect the remote Inx credentials."
	} else {
		...
		if err != nil {
			next.Error = "Unable to load the remote Inx configuration."
...
func (s *Server) configureProviderCredential(ctx context.Context, key string) error {
	...
	if err := s.switchModelLocked(ctx, setup.ModelRef); err != nil {
		...
		s.providerSetup.Error = "The credential was saved, but the Provider could not be activated. Retry or restart Inx Serve."
		s.providerSetupMu.Unlock()
