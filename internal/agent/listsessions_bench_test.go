...
// directory. It is gated behind INX_BENCH=1 so it never runs (or writes
// hundreds of MB) during a normal `go test`.
//
//	INX_BENCH=1 go test ./internal/agent -run TestListSessionsScalingManual -v
func TestListSessionsScalingManual(t *testing.T) {
	if os.Getenv("INX_BENCH") != "1" {
		t.Skip("set INX_BENCH=1 to run the scaling benchmark")
	}
...
//
//	INX_BENCH=1 go test ./internal/agent -run TestColdStartProjectTreeManual -v
func TestColdStartProjectTreeManual(t *testing.T) {
	if os.Getenv("INX_BENCH") != "1" {
		t.Skip("set INX_BENCH=1 to run the cold-start benchmark")
	}
