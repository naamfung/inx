...
func migrationRescueHome(t *testing.T) string {
	...
	t.Setenv("INX_HOME", "")
	t.Setenv("INX_STATE_HOME", filepath.Join(home, "new-state"))
...
func isolateMigrationHome(t *testing.T) string {
	...
	t.Setenv("INX_HOME", filepath.Join(home, "new-inx"))
...
func TestRunLegacyRescueImportsMemory(t *testing.T) {
	home := migrationRescueHome(t)
	legacyRoot := filepath.Join(home, ".inx")
	if err := os.MkdirAll(filepath.Join(legacyRoot, "memory", "global"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacyRoot, "INX.md"), []byte("legacy user memory\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		filepath.Join(config.MemoryUserDir(), "INX.md"),
...
func TestRunLegacyRescueSkipsImplicitSourcesWhenIsolated(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(config.MemoryUserDir(), "INX.md")); !os.IsNotExist(err) {
		t.Fatalf("isolated rescue imported legacy memory, stat err=%v", err)
	}
	joined := strings.Join(notices, "\n")
	if !strings.Contains(joined, "INX_HOME is set; implicit legacy migration is skipped") {
...
func TestRunLegacyRescueCommandImportsFromExplicitInstallDir(t *testing.T) {
	home := isolateMigrationHome(t)
	installRoot := filepath.Join(home, "Custom Inx")
...
func TestMigrateLegacySessionSourcesSkipsCurrentProjectTree(t *testing.T) {
	...
	t.Setenv("INX_HOME", "")
	t.Setenv("INX_STATE_HOME", "")
	if !samePath(config.MemoryUserDir(), filepath.Join(home, ".inx")) {
		t.Skip("current Inx home is not ~/.inx on this platform")
	}
