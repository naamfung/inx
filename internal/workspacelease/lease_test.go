...
func TestWorkspaceLeaseHelperProcess(t *testing.T) {
	if os.Getenv("INX_WORKSPACE_LEASE_HELPER") != "1" {
		return
	}
	root := os.Getenv("INX_WORKSPACE_LEASE_ROOT")
	locks := os.Getenv("INX_WORKSPACE_LEASE_DIR")
	ready := os.Getenv("INX_WORKSPACE_LEASE_READY")
...
func TestCrossProcessLeaseBlocksAndCrashReleases(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"INX_WORKSPACE_LEASE_HELPER=1",
		"INX_WORKSPACE_LEASE_ROOT="+root,
		"INX_WORKSPACE_LEASE_DIR="+locks,
		"INX_WORKSPACE_LEASE_READY="+ready,
	)
