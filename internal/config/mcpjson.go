...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. Inx reads
...
// stdio server; type/url/headers describe a remote one. Inx also accepts
// startup and call timeout fields as Inx policy extensions.
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.inx/config.json.
func legacyConfigPath() string {
	...
	return filepath.Join(home, ".inx", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.inx/config.json and returns its enabled
...
// already declare. inx.toml's [[plugins]] win on a name collision: it is the
// Inx-specific, more explicit of the two, so it overrides the shared,
...
func (c *Config) mergeMCPJSON(entries []PluginEntry) {
	...
	for _, e := range entries {
		if i, exists := index[e.Name]; exists {
			...
			// Within one project, inx.toml remains more specific than the
...
func removeMCPJSONApprovalModes(server map[string]json.RawMessage) error {
	...
	// Remove Inx's retired approval_mode while preserving tool fields owned
