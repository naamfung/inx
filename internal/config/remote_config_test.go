...
func TestProjectConfigCannotOverrideRemote(t *testing.T) {
	...
	if _, ok := cfg.RemoteHost("evil"); ok {
		t.Error("project inx.toml injected a remote host; [remote] must stay user-global")
	}
}
...
passphrase_env = "INX_REMOTE_GPUBOX_PASSPHRASE"
...
func TestUpsertRemoteHostRoundTripsThroughSave(t *testing.T) {
	...
	host := RemoteHostEntry{
		...
		Port:          22,
		User:          "dev",
		PassphraseEnv: "INX_REMOTE_BOX_PASSPHRASE",
