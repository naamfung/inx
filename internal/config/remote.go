...
// after the project merge so a cloned repo's inx.toml can never inject
// hosts, jump chains, or forwards.
type RemoteConfig struct {
	// ImportSSHConfig surfaces ~/.ssh/config aliases in `inx remote import`.
...
// values live in Inx's global .env, never in TOML. identity_file is a
// path — private key material itself is never stored by Inx.
...
// host's Forwards) and let a project inx.toml overwrite the "restored"
// global entries.
