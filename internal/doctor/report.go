...
func Collect(opts Options) Report {
	...
	// while Inx isolation should use INX_HOME. Do not rewrite
...
// home while INX_HOME is unset. Services should keep the real account HOME
// and isolate Inx state with INX_HOME instead of rewriting HOME.
func homeIsolationWarning() string {
	if strings.TrimSpace(os.Getenv("INX_HOME")) != "" {
		return ""
	}
	...
	return "process HOME differs from the OS account home; keep the real account HOME for services and isolate Inx with INX_HOME"
}
