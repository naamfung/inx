...
func TestOwnedShortcutTargetCoversStableAndVersionedEntries(t *testing.T) {
	...
	tests := []struct {
		...
		{filepath.Join(root, "inx-launcher.exe"), true},
		{filepath.Join(root, "Inx.exe"), true},
		{filepath.Join(root, "inx-desktop.exe"), true},
		{filepath.Join(root, "versions", "v1.20.0", "inx-desktop.exe"), true},
		{filepath.Join(root, "versions", "v1.20.0", "inx-cli.exe"), false},
		{filepath.Join(`D:\Apps`, "Inx", "inx-launcher.exe"), false},
	}
...
func TestInxShortcutName(t *testing.T) {
	tests := []struct {
		name string
		want bool
	}{
		{"Inx.lnk", true},
		{"inx launcher.LNK", true},
		{"Inx (2).lnk", true},
		{"Other.lnk", false},
		{"Inx.exe", false},
	}
	for _, test := range tests {
		if got := inxShortcutName(test.name); got != test.want {
			t.Errorf("inxShortcutName(%q) = %v, want %v", test.name, got, test.want)
		}
	}
}
...
func TestOwnedShortcutTargetAcceptsVersionedDesktopThroughJunction(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(versionDir, "inx-desktop.exe"), []byte("desktop"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	target := filepath.Join(junction, "versions", "v1.20.0", "inx-desktop.exe")
...
func TestRepairOwnedShortcutPersistsAppUserModelID(t *testing.T) {
	root := t.TempDir()
	target := filepath.Join(root, "Inx.exe")
	if err := os.WriteFile(target, []byte("launcher"), 0o600); err != nil {
		t.Fatal(err)
	}
	shortcutPath := filepath.Join(root, "Inx.lnk")
...
func TestRepairOwnedShortcutLeavesSeparateInx053InstallUntouched(t *testing.T) {
	currentRoot := t.TempDir()
	legacyRoot := t.TempDir()
	legacyTarget := filepath.Join(legacyRoot, "inx-desktop.exe")
	if err := os.WriteFile(legacyTarget, []byte("legacy tauri desktop"), 0o600); err != nil {
		t.Fatal(err)
	}
	shortcutPath := filepath.Join(t.TempDir(), "Inx.lnk")
	...
	if changed {
		t.Fatal("current install rewrote a shortcut owned by a separate Inx 0.53 installation")
	}
