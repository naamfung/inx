...
func TestBuildMissingSystemPromptFileFallsBackToInlinePrompt(t *testing.T) {
	...
	t.Setenv("INX_HOME", robustTempDir(t))
	writeFile(t, root, "inx.toml", systemPromptFileTestConfig("prompts/missing.md"))
...
func TestBuildNonMissingSystemPromptReadFailureStaysFatal(t *testing.T) {
	...
	root := robustTempDir(t)
	t.Setenv("INX_HOME", robustTempDir(t))
	writeFile(t, root, "inx.toml", systemPromptFileTestConfig("prompts"))
