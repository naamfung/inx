...
// v1.20+ removes product Safe Mode. INX_SAFE_MODE no longer changes boot
...
func TestBuildIgnoresSafeModeEnvForTools(t *testing.T) {
	...
	for _, name := range []string{"install_source", "run_skill", "read_skill"} {
		if !names[name] {
			t.Fatalf("INX_SAFE_MODE must not strip %s", name)
		}
	}
}
...
func TestBuildStillSpawnsExtraPluginsWhenSafeModeEnvSet(t *testing.T) {
	...
	t.Chdir(workspace)
	t.Setenv("INX_SAFE_MODE", "1")
	marker := filepath.Join(plugin.MCPStateDir(config.InxHomeDir(), workspace, "acp-extra"), "started")
	ctrl, err := Build(context.Background(), Options{
		SessionDir: filepath.Join(t.TempDir(), "sessions"),
		Sink:       event.Discard,
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("host-supplied MCP server never spawned; INX_SAFE_MODE must not drop ExtraPlugins")
		}
