...
// regenerate with:
//
//	INX_UPDATE_GOLDEN=1 go test ./internal/boot -run TestGoldenBaseline -count=1
//
...
func TestNormalizeGoldenRootReplacesQuotedWindowsPath(t *testing.T) {
	root := `C:\Users\RUNNER~1\AppData\Local\Temp\inx-test-123`
...
func TestNormalizeGoldenRootReplacesEveryAlias(t *testing.T) {
	prompt := "long=/tmp/inx-long short=/tmp/inx-short"
	got := normalizeGoldenRoot(prompt, "/tmp/inx-long", "/tmp/inx-short")
...
func TestGoldenBaselineNoExtensions(t *testing.T) {
	...
	if os.Getenv("INX_UPDATE_GOLDEN") == "1" {
	...
	for name, want := range artifacts {
		...
		if err != nil {
			t.Fatalf("read golden %s: %v (record it with INX_UPDATE_GOLDEN=1)", name, err)
		}
		if string(got) != string(want) {
			t.Fatalf("golden %s drifted from the pre-refactor baseline (%d bytes golden, %d bytes actual); first diff: %q\n"+
				"If this drift is deliberate, regenerate with INX_UPDATE_GOLDEN=1 and document the cache impact.",
