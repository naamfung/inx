...
func (*toolSourceConnector) Description() string {
	return "Economy mode only: enable optional tools for the current task, including embedded Inx docs. For mcp, pass a configured server name or omit it to list servers. Enabled tools are available on the next model request."
}
