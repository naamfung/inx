...
// the project file is the only config, so isolate INX_HOME: a user-global
...
func TestBuildNoticesMissingAPIKey(t *testing.T) {
	const keyEnv = "INX_MISSING_KEY_FOR_TEST"
	dir := robustTempDir(t)
	t.Chdir(dir)
	writeFile(t, dir, "inx.toml", `
default_model = "x"
...
func TestBuildDoesNotNoticeMissingAPIKeyForNoAuthLoopback(t *testing.T) {
	const keyEnv = "INX_LOCAL_GATEWAY_KEY_FOR_TEST"
	dir := robustTempDir(t)
	t.Chdir(dir)
	t.Setenv(keyEnv, "")
	writeFile(t, dir, "inx.toml", `
...
func TestBuildKeylessDefaultFallsBackToConfiguredProvider(t *testing.T) {
	const keylessEnv = "INX_KEYLESS_DEFAULT_FALLBACK_KEYLESS"
	const configuredEnv = "INX_KEYLESS_DEFAULT_FALLBACK_CONFIGURED"
...
func TestBuildExplicitKeylessModelStillFails(t *testing.T) {
	const keylessEnv = "INX_EXPLICIT_KEYLESS_KEY"
	const configuredEnv = "INX_EXPLICIT_KEYLESS_CONFIGURED"
