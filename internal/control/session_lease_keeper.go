...
// that binds session files for writing (the CLI chat/run commands, `inx
...
const SessionLeaseCloseHint = "close the other Inx window or process first"
...
func SessionInUseMessage(err error) string {
	const fallback = "this session is in use by another Inx window or process"
	...
	fmt.Fprintf(&b, "this session is in use by another Inx process (pid %d", info.PID)
