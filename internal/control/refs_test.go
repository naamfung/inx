...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".inx/attachments/clipboard-20260601-010203.000000.png": true,
		".inx/attachments/clipboard-20260601-010203.000000.yml": true,
		".inx/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".inx/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".inx/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".inx/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".inx", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".inx/attachments/config.yml")
	zipRef := filepath.ToSlash(".inx/attachments/archive.zip")
	pngRef := filepath.ToSlash(".inx/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: inx\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: inx") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".inx", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".inx/attachments/shot.png" {
