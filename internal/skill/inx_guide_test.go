...
func TestInxGuideBuiltinRegistered(t *testing.T) {
	store := skill.New(skill.Options{HomeDir: t.TempDir(), DisableBuiltins: false})
	sk, ok := store.Read("inx-guide")
	if !ok {
		t.Fatal("inx-guide must be registered as a builtin")
	}
...
func TestInxGuideIndexLineOnly(t *testing.T) {
	...
	if !found {
		t.Fatal("inx-guide missing from List")
	}
	idx := skill.IndexBlock(list)
	if !strings.Contains(idx, "inx-guide") {
		t.Fatal("index missing inx-guide line")
	}
	...
	if c := strings.Count(idx, "- inx-guide"); c != 1 {
		t.Fatalf("index lines for inx-guide = %d, want 1", c)
	}
}
...
func TestInxGuideOverriddenByProject(t *testing.T) {
	home := t.TempDir()
	root := t.TempDir()
	store := skill.New(skill.Options{HomeDir: home, ProjectRoot: root})
	// Create project override.
	path, err := store.CreateWithContent("inx-guide", skill.ScopeProject, "---\ndescription: override\nrunAs: inline\n---\nproject body\n")
	if err != nil {
		t.Fatal(err)
	}
	_ = path
	store2 := skill.New(skill.Options{HomeDir: home, ProjectRoot: root})
	sk, ok := store2.Read("inx-guide")
	if !ok {
...
func TestInxGuideDisabled(t *testing.T) {
	store := skill.New(skill.Options{
		HomeDir:       t.TempDir(),
		DisabledNames: []string{"inx-guide"},
	})
	if _, ok := store.Read("inx-guide"); ok {
...
func TestInxGuideIndexStableAcrossCalls(t *testing.T) {
	store := skill.New(skill.Options{HomeDir: t.TempDir()})
