...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "inx-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "inx-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	if !strings.Contains(env, "INX_TEST_VISIBLE=ok") {
...
func TestRunShellPATHCommandFiltersEnvWhenEnabled(t *testing.T) {
	...
	out := runShellPATHCommand(context.Background(), "/bin/sh", []string{"-c", `printf 'tok=%s' "${INX_TEST_SECRET_TOKEN:-none}"`})
