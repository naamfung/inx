...
// stateRootFor builds a fake Inx state root with the two guarded session
...
func TestSessionDataGuardDeniesSessionStores(t *testing.T) {
	...
	for _, target := range []string{
		...
		} else if !strings.Contains(err.Error(), "Inx's own session/state data") {
...
func TestSessionDataGuardZeroValueUnconfined(t *testing.T) {
	...
	if hint := g.CommandHint("", "rm -rf ~/.inx/sessions"); hint != "" {
...
func TestSessionDataGuardCommandHintEnvVarForm(t *testing.T) {
	...
	for _, cmd := range []string{
		`python3 -c "open('$HOME/.inx/sessions/x.jsonl','w')"`,
		"rm ${HOME}/.inx/projects/slug/sessions/y.jsonl",
	} {
		if hint := g.CommandHint("", cmd); hint == "" {
...
func TestBashAppendsSessionDataHint(t *testing.T) {
	...
	if !strings.Contains(out, "WARNING: this command referenced Inx's own session/state data") {
