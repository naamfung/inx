...
func TestDownloadCLIFromBaseVerifiesAndExtracts(t *testing.T) {
	binary := []byte("inx-binary")
	archive := testCLIArchive(t, binary)
	digest := sha256.Sum256(archive)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v1.2.3/inx-linux-arm64.tar.gz":
			_, _ = w.Write(archive)
		case "/v1.2.3/SHA256SUMS":
			_, _ = fmt.Fprintf(w, "%s  inx-linux-arm64.tar.gz\n", hex.EncodeToString(digest[:]))
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestDownloadCLIFromBaseRejectsChecksumMismatch(t *testing.T) {
	archive := testCLIArchive(t, []byte("inx-binary"))
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v1.2.3/inx-linux-amd64.tar.gz" {
			_, _ = w.Write(archive)
			return
		}
		_, _ = fmt.Fprintf(w, "%064d  inx-linux-amd64.tar.gz\n", 0)
	}))
	defer server.Close()
...
func testCLIArchive(t *testing.T, binary []byte) []byte {
	...
	tw := tar.NewWriter(gz)
	if err := tw.WriteHeader(&tar.Header{Name: "inx", Mode: 0o755, Size: int64(len(binary)), Typeflag: tar.TypeReg}); err != nil {
		t.Fatal(err)
	}
