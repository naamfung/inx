...
// hook machine resolves global hooks from the platform Inx home
// (config.InxHomeDir → settings.json), not a doubled .inx segment
// (#7420). INX_HOME is set explicitly so the resolved path is
