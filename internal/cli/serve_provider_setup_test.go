...
const serveMissingKeyHelperEnv = "INX_TEST_SERVE_MISSING_KEY_HELPER"
...
func TestServeStartsWithMissingProviderKey(t *testing.T) {
	if os.Getenv(serveMissingKeyHelperEnv) == "1" {
		...
		if err := os.Setenv("INX_HOME", os.Getenv("INX_TEST_SERVE_HOME")); err != nil {
			os.Exit(2)
		}
		code := runServe([]string{
			"--model", "remote-demo/model-a",
			"--addr", "127.0.0.1:0",
			"--port-file", os.Getenv("INX_TEST_SERVE_PORT_FILE"),
			"--auth", "token",
...
models = ["model-a"]
default = "model-a"
api_key_env = "INX_TEST_REMOTE_MISSING_KEY"
`
	...
	cmd.Env = replaceServeTestEnv(os.Environ(),
		...
		"INX_HOME="+home,
		"INX_TEST_SERVE_HOME="+home,
		"INX_CREDENTIALS_STORE=file",
		"INX_TEST_REMOTE_MISSING_KEY=",
		"INX_TEST_SERVE_PORT_FILE="+portFile,
	)
	...
	if !bytes.Contains(body, []byte("Inx Provider Setup")) {
