...
// CLI defaults to a FileStore backed by .inx/tasks under the
// project directory.
...
func taskCommand(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: inx task <list|show|monitor|status|events|stop|cancel|requeue|open-session|tmux> [flags]")
		return 2
	}
	store := taskStore
	if store == nil {
		store = taskmonitor.NewFileStore(".inx/tasks")
	}
	switch args[0] {
	case "list":
...
func taskMonitorCommand(store taskmonitor.Store, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: inx task monitor <list|status|events|stop|cancel|requeue|open-session> [flags]")
		return 2
	}
	switch args[0] {
	case "list":
...
func taskTmuxCmd(store taskmonitor.Store, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: inx task tmux <attach|status|open|detach>")
		return 2
	}
	a := taskmonitor.NewTmuxAdapter(store, ".inx/tasks")
	switch args[0] {
	case "attach":
...
func taskTmuxAttachCmd(a *taskmonitor.TmuxAdapter, args []string) int {
	...
	if code != 0 || fs.Arg(0) == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task tmux attach <id> --json [--dir DIR] [--session NAME]")
		return 2
	}
...
func taskTmuxStatusCmd(a *taskmonitor.TmuxAdapter, args []string) int {
	...
	if code != 0 || fs.Arg(0) == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task tmux status <id> --json [--dir DIR]")
		return 2
	}
...
func taskTmuxOpenCmd(a *taskmonitor.TmuxAdapter, args []string) int {
	...
	if code != 0 || fs.Arg(0) == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task tmux open <id> --json [--dir DIR]")
		return 2
	}
...
func taskTmuxDetachCmd(a *taskmonitor.TmuxAdapter, args []string) int {
	...
	if code != 0 || fs.Arg(0) == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task tmux detach <id> --json [--dir DIR]")
		return 2
	}
...
func taskStatusCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task status <id> --json [--dir DIR]")
		return 2
	}
...
func taskEventsCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task events <id> --json|--jsonl [--dir DIR] [--after N] [--follow]")
		return 2
	}
...
func taskStopCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task stop <id> --expected-version N --json")
		return 2
	}
...
func taskCancelCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task cancel <id> --expected-version N --json")
		return 2
	}
...
func taskRequeueCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task requeue <id> --expected-version N --json")
		return 2
	}
...
func taskOpenSessionCmd(store taskmonitor.Store, args []string) int {
	...
	id := fs.Arg(0)
	if id == "" {
		fmt.Fprintln(os.Stderr, "usage: inx task open-session <id> --json")
		return 2
	}
