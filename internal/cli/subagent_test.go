...
func TestSubagentProfileCLIRejectsBuiltinCollisionAndRichSkillEdit(t *testing.T) {
	...
	path := filepath.Join(project, ".inx", "skills", "rich", skill.SkillFile)
	...
	roPath := filepath.Join(project, ".inx", "skills", "readonly-agent", skill.SkillFile)
...
func TestSubagentProfileCLIRejectsReservedAndCustomCommandNames(t *testing.T) {
	...
	t.Cleanup(func() { _ = os.Chdir(original) })
	commandPath := filepath.Join(project, ".inx", "commands", "formatter.md")
...
func TestSubagentProfileCLIEditBuiltinModelOverride(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:            "chat",
		APIKeyEnv:        "INX_SUBAGENT_OFFLINE_KEY",
...
func TestRootHelpListsSubagentCommand(t *testing.T) {
	...
	if !strings.Contains(out, "inx subagent <list|create|edit|delete|try|run>") {
