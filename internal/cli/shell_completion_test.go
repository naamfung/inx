...
func TestCLICompletionListsRootAndNestedCommands(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 1, []string{"inx", "co"}, values)
	if want := []string{"config", "completion"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("root completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"inx", "mcp", "b"}, values)
	if want := []string{"browse"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("mcp subcommand completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "remote", "serve", "st"}, values)
...
func TestCLICompletionListsCommandFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"inx", "run", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"inx", "--d"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "mcp", "add", "--h"}, values)
...
func TestCLICompletionUsesConfiguredModelsAndSessionIDs(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"inx", "run", "--model", "deep"}, values)
	if want := []string{"deepseek/deepseek-chat"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 1, []string{"inx", "--model=mi"}, values)
	if want := []string{"--model=mimo/mimo-v2"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("inline model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"inx", "--resume", "b"}, values)
...
func TestCompletionCommandPrintsShellScripts(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh", "fish"} {
		t.Run(shell, func(t *testing.T) {
			...
			if !strings.Contains(out, "inx completion __complete") {
				t.Fatalf("completion %s script does not route to the shared registry:\n%s", shell, out)
			}
			if shell == "fish" && strings.Contains(out, "complete -c inx -f ") {
...
func TestCLICompletionTaskNestedAndRunAblate(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"inx", "task", "st"}, values)
	for _, want := range []string{"status", "stop"} {
		if !containsCompletionValue(got, want) {
			t.Fatalf("task prefix st missing %q: %v", want, got)
		}
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"inx", "task", "mon"}, values)
	if !containsCompletionValue(got, "monitor") {
		t.Fatalf("task monitor missing: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "monitor", "st"}, values)
	if !containsCompletionValue(got, "status") || !containsCompletionValue(got, "stop") {
		t.Fatalf("task monitor st = %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"inx", "run", "--a"}, values)
...
func TestCLICompletionOptionalResumeThenFlag(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"inx", "--resume", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"inx", "--resume=a"}, values)
...
func TestCLICompletionRunServeResumeRequiresValue(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"inx", "run", "--resume", "--m"}, values)
	if containsCompletionValue(got, "--model") {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "run", "--resume", "b"}, values)
	if want := []string{"beta-session"}; !reflect.DeepEqual(got, want) {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "serve", "--resume", "a"}, values)
	if len(got) != 0 {
	...
	got = cliCompletionCandidatesWithValues(root, 2, []string{"inx", "run", "--resume=b"}, values)
...
func TestCLICompletionTaskPerOperationFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "status", "--p"}, values)
	if containsCompletionValue(got, "--project-root") {
		t.Fatalf("task status must not offer --project-root: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "status", "--j"}, values)
	if !containsCompletionValue(got, "--json") {
		t.Fatalf("task status missing --json: %v", got)
	}

	// events has --jsonl/--after/--follow.
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "events", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "stop", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"inx", "task", "list", "--p"}, values)
	if !containsCompletionValue(got, "--project-root") {
		t.Fatalf("task list missing --project-root: %v", got)
	}

	// tmux attach has --session.
	got = cliCompletionCandidatesWithValues(root, 4, []string{"inx", "task", "tmux", "attach", "--s"}, values)
...
func TestCLICompletionPathFlagReturnsEmptyForShellFallback(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"inx", "run", "--dir", "do"}, values)
	if len(got) != 0 {
