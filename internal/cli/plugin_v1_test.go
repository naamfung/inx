...
// the test Inx home and registers it in plugin state.
func installV1RuntimePlugin(t *testing.T, home string) string {
  ...
  "runtime": {
	...
	writePluginTestFile(t, filepath.Join(root, "themes", "neon.inx-theme"), "theme bytes")
	writePluginTestFile(t, filepath.Join(root, "bin", "example"), "#!/bin/sh\n")
	if err := pluginpkg.Upsert(home, pluginpkg.InstalledPlugin{
		Name: "example", Root: "plugins/example", Version: "1.0.0", ManifestKind: "inx", Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	return root
...
func TestPluginDoctorReportsMissingV1PathsAsWarnings(t *testing.T) {
  ...
  "contributes": {
	...
	if err := pluginpkg.Upsert(home, pluginpkg.InstalledPlugin{
		Name: "gap", Root: "plugins/gap", ManifestKind: "inx", Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(out, `theme glob "themes/*.inx-theme" matched no files`) {
