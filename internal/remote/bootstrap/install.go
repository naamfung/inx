...
// ensureBinary resolves a usable inx binary on the remote host per the
...
func ensureBinary(ctx context.Context, conn Conn, fs *sftpfs.FS, opts Options, home, goos, goarch string, paths StatePaths) (bin, version string, err error) {
	...
	switch strategy {
	case InstallNever:
		return "", "", fmt.Errorf("bootstrap: inx not found on remote and serve_install = never")
	case InstallNPM:
	...
	default: // auto: try npm, packaged same-platform upload, then verified release upload
		if b, v, nerr := installViaNPM(ctx, conn, opts.MinVersion); nerr == nil {
			...
			if opts.LocalBinary != "" && opts.LocalGOOS == goos && opts.LocalGOARCH == goarch {
				...
				attempts = append(attempts, errors.New("bootstrap: no local Inx CLI is available for upload"))
			} else {
...
// locate finds an existing inx and returns it only if its serve command
...
func installViaNPM(ctx context.Context, conn Conn, minVersion string) (bin, version string, err error) {
	res, err := conn.Exec(ctx, "npm i -g inx 2>&1")
	if err != nil {
	...
	if loc == "" {
		return "", "", fmt.Errorf("bootstrap: inx not found after npm install (check remote PATH / npm prefix)")
	}
	return loc, ver, nil
}

// installViaUpload uploads the local inx binary when the remote platform
...
func installViaUpload(ctx context.Context, conn Conn, fs *sftpfs.FS, opts Options, home, goos, goarch, uploaded string) (bin, version string, err error) {
	if opts.LocalBinary == "" {
		return "", "", fmt.Errorf("bootstrap: upload strategy needs the local inx binary path")
	}
