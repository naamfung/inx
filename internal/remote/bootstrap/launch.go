...
// state. All are under ~/.inx/remote.
type StatePaths struct {
	Dir       string // ~/.inx/remote
	StateJSON string
	TokenFile string
	LogFile   string
	PortFile  string
...
func StopCommand(pid int, p StatePaths) string {
	return fmt.Sprintf(
		"T=%s; P=%s; ours() { A=$(ps -p %d -o args= 2>/dev/null || ps -p %d -o command= 2>/dev/null); "+
			"case \"$A\" in *inx*serve*\"$T\"*\"$P\"*) return 0;; *) return 1;; esac; }; "+
...
// looks like a inx serve process. Checking the args (not just `kill -0`)
...
func ServeAliveCommand(pid int, p StatePaths) string {
	return fmt.Sprintf(
		"T=%s; P=%s; kill -0 %d 2>/dev/null || { echo 0; exit 0; }; "+
			...
			"case \"$A\" in *inx*serve*\"$T\"*\"$P\"*) echo 1;; *) echo 0;; esac",
...
// LocateCommand probes for a usable inx binary. It prints three lines:
...
func LocateCommand(uploadedBin string) string {
	return fmt.Sprintf(
		"BIN=\"$(command -v inx 2>/dev/null)\"; "+
			"if [ -z \"$BIN\" ] && [ -x %s ]; then BIN=%s; fi; "+
			"if [ -z \"$BIN\" ]; then P=\"$(npm prefix -g 2>/dev/null)\"; if [ -n \"$P\" ] && [ -x \"$P/bin/inx\" ]; then BIN=\"$P/bin/inx\"; fi; fi; "+
			"echo \"$BIN\"; "+
