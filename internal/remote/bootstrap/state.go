...
// remoteDir is the ~/.inx/remote directory given the resolved remote home.
func remoteDir(home string) string {
	return path.Join(home, ".inx", store.RemoteDirName)
}
...
// uploadedBinPath is the fallback location for an uploaded inx binary.
func uploadedBinPath(home string) string {
	return path.Join(remoteDir(home), store.RemoteBinDirName, "inx")
}
