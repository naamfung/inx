...
func TestTmuxAdapterAttachIdempotent(t *testing.T) {
	...
	mock := &tmuxMock{}
	a := NewTmuxAdapterWithRunner(s, ".inx/tasks", mock)
...
func TestTmuxAdapterBoundsGeneratedSessionName(t *testing.T) {
	...
	a := NewTmuxAdapterWithRunner(s, ".inx/tasks", &tmuxMock{})
...
func TestTmuxAdapterUnavailableDoesNotChangeTask(t *testing.T) {
	...
	root := t.TempDir()
	seedTmuxTask(t, s, root)
	a := NewTmuxAdapterWithRunner(s, ".inx/tasks", nil)
...
func TestTmuxAdapterStaleAndDetach(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".inx/tasks/.tmux/t1.json")); !os.IsNotExist(err) {
		t.Fatalf("mapping was not removed: %v", err)
	}
}

func TestTmuxAdapterRejectsTaskPathTraversal(t *testing.T) {
	a := NewTmuxAdapterWithRunner(NewInMemoryStore(), ".inx/tasks", &tmuxMock{})
...
func TestTmuxAdapterRejectsSymlinkMappingDirectory(t *testing.T) {
	...
	a := NewTmuxAdapterWithRunner(s, ".inx/tasks", attachRunner)
	...
	readRunner := &tmuxMock{}
	a = NewTmuxAdapterWithRunner(s, ".inx/tasks", readRunner)
...
func TestTmuxAdapterRejectsForgedMappingIdentityWithoutTmuxCalls(t *testing.T) {
	...
	path := filepath.Join(project, ".inx", "tasks", ".tmux", "t1.json")
...
func TestTmuxAdapterWritesPrivateMappingFiles(t *testing.T) {
	...
	for path, want := range map[string]os.FileMode{
		...
		filepath.Join(project, ".inx", "tasks", ".tmux"):            0o700,
		filepath.Join(project, ".inx", "tasks", ".tmux", "t1.json"): 0o600,
	} {
