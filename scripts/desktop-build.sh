...
if [ "$os" = windows ]; then
	...
	windows_host_include="$ROOT/desktop/build/windows/installer/inx_host.nsh"
	case "$(uname -s 2>/dev/null || printf '%s' unknown)" in
		Darwin* | Linux* | FreeBSD*)
			printf '%s\n' '!define INX_UNINST_FINALIZE '\''/bin/cp -f "%1" "inx-uninstall.exe"'\''' >"$windows_host_include"
			;;
		*)
			printf '%s\n' '!define INX_UNINST_FINALIZE '\''cmd.exe /C copy /Y "%1" "inx-uninstall.exe" >NUL'\''' >"$windows_host_include"
			;;
	esac
