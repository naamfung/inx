...
$shortcutName = "Inx Desktop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$workRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("inx-scoop-launch-" + [guid]::NewGuid().ToString("N"))
...
$archivePath = Join-Path $fixtureRoot "inx-windows-portable.zip"
...
try {
    ...
    $desktopPath = Join-Path $packageRoot "versions\$activeVersion\inx-desktop.exe"
...
func main() {
	marker := os.Getenv("INX_SCOOP_MARKER")
...
{
  "version": "$appVersion",
  "description": "Inx Scoop integration fixture",
  "homepage": "https://github.com/esengine/DeepSeek-Inx",
  "license": "Apache-2.0",
  "url": "$archiveUrl",
  "hash": "$archiveHash",
  "shortcuts": [
    ["inx-launcher.exe", "$shortcutName", "launch --detach"]
  ]
}
"@
    ...
    $env:INX_SCOOP_MARKER = $markerPath
    ...
    $expectedTarget = Join-Path $currentPath "inx-launcher.exe"
    ...
    $expectedDesktop = Join-Path $versionPath "versions\$activeVersion\inx-desktop.exe"
