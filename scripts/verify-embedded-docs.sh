...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/inx-docs-verify.XXXXXX")"
cleanup() {
	case "$tmp_dir" in
	*/inx-docs-verify.*) rm -rf -- "$tmp_dir" ;;
...
ldflags="-s -w -X main.version=$version -X main.gitCommit=$git_commit -X main.buildTimeUTC=$build_time_utc -X inx/internal/productdocs.linkedVersion=$version -X inx/internal/productdocs.linkedRevision=$revision"
(
	cd "$repo_root"
	CGO_ENABLED=0 go build -trimpath -ldflags="$ldflags" -o "$binary" ./cmd/inx
)

mkdir -p "$tmp_dir/inx-home"
manifest="$(
	INX_HOME="$tmp_dir/inx-home" "$binary" docs-manifest \
