...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/inx-release-postflight.XXXXXX")"
cleanup() {
	case "$tmp_dir" in
	*/inx-release-postflight.*) rm -rf -- "$tmp_dir" ;;
...
jq -e '
  ...
  ([.assets[].name] as $names |
    ["SHA256SUMS", "inx-darwin-amd64.tar.gz", "inx-darwin-arm64.tar.gz",
     "inx-linux-amd64.tar.gz", "inx-linux-arm64.tar.gz",
     "inx-windows-amd64.zip", "inx-windows-arm64.zip"] |
...
jq -e '
  ...
  ([.assets[].name] as $names |
    ...
    (["Inx-darwin-universal.dmg", "Inx-linux-amd64.deb",
      "Inx-linux-amd64.tar.gz", "Inx-windows-amd64-installer.exe",
...
for attempt in $(seq 1 "$attempts"); do
	latest="$(npm view inx dist-tags.latest 2>/dev/null || true)"
