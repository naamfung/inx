// Package extension is the Go SDK for Inx extension sidecars speaking
// Extension Protocol v1 over stdio. An extension is a separate process: the
// Inx host launches it, sends extension/initialize first, drives
...
// internal/extension/protocol/schema.generated.json in the Inx
// repository.
package extension
...
func Serve(ctx context.Context, h Handler, opts Options) error {
	...
	logger := opts.Logger
	if logger == nil {
		logger = log.New(os.Stderr, "inx-extension: ", log.LstdFlags)
	}
	s := &server{
		handler: h,
		opts:    opts,
