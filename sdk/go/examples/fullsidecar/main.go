// Command fullsidecar is the reference Inx extension sidecar: one small
...
// Environment:
//
//	INX_PLUGIN_NAME   plugin ID, set by the host at launch (provider
...
func main() {
	...
	id := strings.TrimSpace(os.Getenv("INX_PLUGIN_NAME"))
	if id == "" {
		id = defaultPluginID
	}
...
func (p *plugin) interceptSystemPrompt(_ context.Context, payload json.RawMessage) (*extension.InterceptResult, error) {
	...
	owned := "You are Inx running under the fullsidecar demo strategy.\n\n" +
	...
	go func() {
		...
		script := []extension.StreamChunk{
			...
			{Type: extension.ChunkToolCall, ToolCall: &extension.ProviderToolCall{
				ID: "call-1", Name: "lookup", Arguments: `{"query":"inx"}`,
			}},
