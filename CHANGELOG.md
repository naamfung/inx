...
`0.x` TypeScript history lives on the [`v1`](https://github.com/esengine/DeepSeek-Inx/tree/v1)
branch.
...
- Added an authenticated, loopback-only Provider setup page for `inx
  serve`. A Serve whose selected Provider is missing its API key now remains
  reachable, stores the submitted key in that host's Inx credential file,
...
- Remote SSH workspaces now open as a standalone remote web window again.
  ...
  reuses the remote `inx serve`, tunnels its loopback port, and opens the
...
- Full bilingual release notes:
  <https://inx.io/changelog/v1.20.0/> ·
  [GitHub release](https://github.com/esengine/DeepSeek-Inx/releases/tag/desktop-v1.20.0).
...
[bilingual release notes](https://inx.io/changelog/) and on the
[GitHub releases page](https://github.com/esengine/DeepSeek-Inx/releases).
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`esengine/inx` tap), and release archives;
...
[1.20.0]: https://github.com/esengine/DeepSeek-Inx/releases/tag/desktop-v1.20.0
