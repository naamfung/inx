...
> WeChat, and QQ bots, how to use Inx from IM, and how approvals, Ask
...
After a bot is connected, you can send Inx messages from Feishu, Lark,
WeChat, or QQ. The desktop app or `inx bot start` process handles the
model, tools, permissions, sandboxing, and local context, then sends progress
and results back to the IM channel.

Common uses:

- Ask Inx to inspect code, read docs, explain errors, or summarize findings.
...
- **CLI runtime**: run `inx bot start` for a headless long-lived process.
...
The normal `inx run` command does not automatically start the IM gateway.
Remote bot behavior is active only while the desktop bot runtime is running or
while a `inx bot start` process is alive.

## Connect the four channels

Open the Inx desktop app and go to **Settings -> Bots**. In **Add IM Bot**,
...
CommonMark formatting. If a card is too large for the platform limit, Inx
...
inx bot doctor
inx bot doctor --deep
inx bot start --channels qq,feishu,lark,weixin --dir /path/to/project
```
...
- `credential.app_id`, `credential.app_secret_env`, `credential.account_id`,
  ...
  tokens. Secrets stay in environment variables or the Inx user credentials
  store.
...
`inx bot pairing approve <code>` before the sender can drive the bot; when
...
```sh
export INX_BOT_CONTROL_TOKEN="change-me"

curl -H "Authorization: Bearer $INX_BOT_CONTROL_TOKEN" \
  http://127.0.0.1:37913/status

curl -X POST http://127.0.0.1:37913/send \
  -H "Authorization: Bearer $INX_BOT_CONTROL_TOKEN" \
...
sequenceDiagram
  ...
  participant R as "Inx desktop or bot start"
...
| `/projects [query]` | List indexed project workspaces | `/projects inx` |
...
`inx bot doctor --deep` reports queue, pairing, and role diagnostics.
...
- `/search all <query>` searches file contents across indexed project roots.
  Inx uses `rg` when available and falls back to a bounded Go scanner.
...
current workspace's `.inx/attachments` directory and passes them to
Inx as `@.inx/attachments/...` references. If an attachment cannot be
...
Inx bots use the same permission system as the desktop app. Ask mode is the
...
No. A normal Inx app upgrade or overwrite install does not require
rebinding.

Bindings are stored in the user's Inx data, not inside the app bundle:

- Bot connections, remote IDs, allowlists, model choices, and approval modes are
  stored in the user config.
- Feishu and Lark secrets are stored in Inx's global
  `<Inx home>/.env`, shared by CLI and desktop.
- The WeChat scanned account token is stored in the Inx user data
  directory.
...
- The Inx user config directory was deleted.
...
| Connected but no reply | Make sure the desktop bot runtime or `inx bot start` process is running, the bot connection is enabled, and the sender ID is allowlisted, paired, or access is open. |
