...
The Ask / Auto / Yolo control under the desktop composer sets how Inx handles tool permission approvals. All three modes stay visible so you can switch directly without relying on a shortcut or settings page.

Tool permission is independent of collaboration mode:

- **Collaboration / runtime mode** decides how Inx advances the task (lightweight, balanced, or delivery-first).
...
Ask is the most conservative tool-permission mode. When Inx needs approval for a tool call, an approval card appears so you can allow once, allow for the session, always allow, or deny.
...
Headless `inx run` has no approval card to answer. Its default Ask posture
...
- When entered via shortcut, Inx remembers the previous Ask/Auto baseline and restores it on the next toggle.
