...
with `runAs: subagent`: Inx starts an isolated child agent, gives it the
...
`.inx/skills/<name>/SKILL.md`; global profiles are stored under the
Inx home Skill directory described in
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  inx subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. Inx refuses a name
...
inx subagent try reviewer "review the current diff"
...
git diff | inx subagent run reviewer --max-steps 20
```
...
apply. Ordinary `inx run` remains a plain one-shot task entry point and
...
inx subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
inx subagent edit <name> [--description TEXT]
...
```bash
inx subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `inx subagent create` share
...
`inx subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when Inx resolves the effective model.
