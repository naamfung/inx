...
Context Engine v2 为 Inx 提供两个权限不同的持久上下文层：
...
| 放在哪里 | 适合存放 | 示例 |
| --- | --- | --- |
| `AGENTS.md`、`INX.md` 或 `CLAUDE.md` | 每个相关回合都必须存在的规则 | 必跑测试、仓库边界、评审约定 |
...
Inx 识别 `INX.md`、`AGENTS.md`、`CLAUDE.md`，以及对应的 `.local.md`
变体。它先加载 Inx home 下的用户全局指令，再从 workspace root 逐级走到目标路径；
...
每个真实用户回合开始前，Inx 会用原始用户消息搜索 active facts。宿主追加给 provider
...
普通路径零配置。只有同时满足以下条件时，Inx 才可以自动创建一条新记忆：
...
能力时，Inx 不会回退读取桌面机器的本地 session 或 memory。
