...
Inside `inx`, use the slash command:
...
`inx.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
inx run "explain this module"
```
...
```text
./inx.toml > user config.toml > built-in defaults
```
...
Inx adds the same small transient `<reasoning-language>` block for that
...
When set to `zh` or `en`, Inx always adds a small transient
