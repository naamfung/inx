...
2. Install it directly over the current copy (Windows: double-click; macOS:
   replace `Inx.app`). Do not uninstall first: keeping the existing install
   root lets the compatibility migrator prove which stale transaction it owns.
3. Start Inx once and confirm **Settings > Updates** shows the installed
...
2. If that button is missing or fails, quit Inx fully and start it once so
...
3. If in-app update still fails, download the latest signed installer from the
   ...
   uninstalling first.
4. On macOS, also allow Inx under System Settings → Privacy & Security →
   App Management when the dialog appears; a leftover
   `Inx.app.inx-update-backup` that TCC will not let the app remove
   may still require the official installer path.

If the Windows installer reports `Inx layout activation failed`, expand
the installer details and copy the lines under `Inx layout activator
...
After the replacement window becomes visible, Inx commits only the exact
