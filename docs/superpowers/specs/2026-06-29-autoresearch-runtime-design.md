...
Inx already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.inx/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing Inx gates still apply.
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.inx/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.inx/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.inx/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .inx/autoresearch/<task-id>/`
...
- explicit `.inx/autoresearch/<task-id>/` resumes existing state
...
No existing `.inx/autoresearch` task should be deleted or rewritten without
