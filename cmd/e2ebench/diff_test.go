...
func TestEnv(t *testing.T) {
	if got := os.Getenv("INX_E2E_ENV"); got != "ok" {
		t.Fatalf("INX_E2E_ENV = %q", got)
	}
}
`)

	ok, out := runTests(repo, "GOWORK=off INX_E2E_ENV=ok go test", []string{"./..."})
	if !ok {
...
func TestRunTestsRejectsDynamicEnvAssignment(t *testing.T) {
	ok, out := runTests(t.TempDir(), "INX_E2E_ENV=$(echo ok) go test", []string{"./..."})
	if ok {
