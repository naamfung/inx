...
function bindings(db: D1Database): Bindings {
  return {
    DB: db,
    APP_ORIGIN: "https://inx.io",
    ALLOWED_ORIGINS: "https://inx.io",
    ID_ORIGIN: "https://id.inx.io",
  };
}
...
describe("forum public API", () => {
  it("maps invalid query input to a client error", async () => {
    ...
    const response = await app.request("https://forum.inx.io/topics?sort=invalid", {}, bindings(db));
  ...
  it("selects public handles instead of stored author emails", async () => {
    ...
    const response = await app.request("https://forum.inx.io/topics", {}, bindings(db));
