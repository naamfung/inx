// Dashboard authorization. Identity comes from id.inx.io (the shared
...
// The id.inx.io session cookie is scoped to `.inx.io`, so the browser
...
function idOrigin(env: Env): string {
  return (env.ID_ORIGIN ?? "https://id.inx.io").replace(/\/$/, "");
}

function appOrigin(env: Env): string {
  return (env.APP_ORIGIN ?? "https://inx.io").replace(/\/$/, "");
}
...
async function identityCacheKey(token: string): Promise<Request> {
  ...
  return new Request(`https://id-cache.inx.io/${hex}`);
}
...
// Ends the shared id.inx.io session (best-effort) and returns a Set-Cookie
...
export async function sharedLogout(request: Request, env: Env): Promise<string> {
  ...
  return `${SHARED_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Domain=.inx.io; Max-Age=0`;
}
