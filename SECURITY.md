...
Inx security fixes are prioritized for the currently developed Go rewrite
...
- Affected Inx version, commit, operating system, and installation method.
...
Inx is a local coding agent. Many features intentionally operate on the
...
Inx supports `@path` references so users can include local files and images
...
- A local user intentionally asks Inx to read, edit, or send their own
