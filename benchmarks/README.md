...
| `max_steps` | int | yes | Agent tool-call cap; passed through as `--max-steps` to `inx run`. |
...
Prerequisites: a `inx` binary (or `go run ./cmd/inx` …) with a
configured provider. The harness invokes the agent as
`inx run --auto --metrics <path> [--model NAME] [--max-steps N] [--profile delivery] [--ablate ARM] <prompt>`
...
| `-bin` | `inx` | Path to the inx binary. |
...
| `-trajectories` | *(none)* | Suite mode: write one `<task-id>.trajectory.jsonl` per task into this directory (the agent's full event stream with timestamps — see `inx run --trajectory`). The report gains a time-attribution line (tools vs. model) and each JSON result a `trajectory` digest. |
...
go run ./cmd/e2ebench -mode swebench \
  ...
  -network inx-eval -proxy http://127.0.0.1:8080
```
...
- [`docs/CLI.md`](../docs/CLI.md) — the `inx run` flags the e2e harness
