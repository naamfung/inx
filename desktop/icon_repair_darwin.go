...
int inx_repair_alias(const char *alias_path, const char *current_app_path,
                          int allow_broken, char **error_message);
int inx_write_alias(const char *target_path, const char *alias_path,
                         char **error_message);
int inx_resolve_alias(const char *alias_path, char **target_path,
                           char **error_message);
int inx_is_inx_bundle_url_string(const char *url_string,
                                           char **error_message);
*/
import "C"
...
func repairMacDesktopAliases(desktopDir, currentApp string) error {
	...
	for _, entry := range entries {
		...
		allowBroken := inxExactAliasName(entry.Name())
...
func inxExactAliasName(name string) bool {
	name = strings.TrimSpace(name)
	return strings.EqualFold(name, "Inx") || strings.EqualFold(name, "Inx.app")
}
...
func repairMacAlias(aliasPath, currentApp string, allowBroken bool) (bool, error) {
	...
	result := C.inx_repair_alias(aliasCString, appCString, allow, &nativeErr)
	if nativeErr != nil {
...
func writeMacAlias(targetPath, aliasPath string) error {
	...
	defer C.free(unsafe.Pointer(aliasCString))
	var nativeErr *C.char
	result := C.inx_write_alias(targetCString, aliasCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result != 0 {
...
func resolveMacAlias(aliasPath string) (string, error) {
	...
	result := C.inx_resolve_alias(aliasCString, &target, &nativeErr)
	if target != nil {
...
func macURLReferencesInxBundle(rawURL string) (bool, error) {
	urlCString := C.CString(rawURL)
	defer C.free(unsafe.Pointer(urlCString))
	var nativeErr *C.char
	result := C.inx_is_inx_bundle_url_string(urlCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result < 0 {
