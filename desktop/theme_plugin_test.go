...
// contributes.themes globbing themes/*.inx-theme, plus the
...
func installPluginThemeFixture(t *testing.T, home, pluginName string, enabled bool, themes []pluginThemeFixture) string {
	...
	manifestJSON := fmt.Sprintf(
		`{"apiVersion":%q,"name":%q,"version":"1.0.0","contributes":{"themes":["themes/*.inx-theme"]}}`,
...
func TestPluginThemeDiscoveryAndList(t *testing.T) {
	...
	pluginRoot := installPluginThemeFixture(t, home, "themery", true, []pluginThemeFixture{
		{fileName: "neon.inx-theme", manifest: testPluginThemeManifest("neon-dusk", "Neon Dusk"), withImage: true},
	})
	app := NewApp()
	...
	if _, err := os.Stat(filepath.Join(pluginRoot, "themes", "neon.inx-theme")); err != nil {
		t.Fatal(err)
	}
}
...
func TestPluginThemeInvalidSkippedWithWarning(t *testing.T) {
	...
	installPluginThemeFixture(t, home, "themery", true, []pluginThemeFixture{
		{fileName: "neon.inx-theme", manifest: testPluginThemeManifest("neon-dusk", "Neon Dusk")},
		{fileName: "broken.inx-theme", manifest: nil}, // invalid ZIP
	})
	app := NewApp()
	...
	if len(view.Warnings) == 0 || !strings.Contains(view.Warnings[0], "broken.inx-theme") {
	...
	for _, w := range exp.Warnings {
		if strings.Contains(w, "broken.inx-theme") {
			found = true
		}
	}
	if !found {
