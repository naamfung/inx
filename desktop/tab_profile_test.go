...
effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "inx.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(root, "inx.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAutoResearchStatusSurfaceForActiveTab(t *testing.T) {
	...
	heartbeatPath := filepath.Join(root, ".inx", "autoresearch", current.TaskID, "logs", "heartbeat.jsonl")
...
func TestAutoResearchFindingsAreLoadedOnDemand(t *testing.T) {
	...
	findingsPath := filepath.Join(root, ".inx", "autoresearch", current.TaskID, "state", "findings.jsonl")
