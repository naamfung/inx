...
// Plugin themes are .inx-theme packs contributed by ENABLED installed
...
type pluginTheme struct {
	...
	path       string // absolute path of the .inx-theme ZIP
...
func discoverPluginThemes() ([]pluginTheme, []string) {
	pkgs, warnings := pluginpkg.LoadInstalled(config.InxHomeDir())
	var out []pluginTheme
