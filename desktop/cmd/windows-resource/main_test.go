...
func TestStampExecutableSupportsWindowsReleaseArchitectures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.arch, func(t *testing.T) {
			...
			executable := filepath.Join(dir, "inx-launcher.exe")
			...
			opts := resourceOptions{
				...
				fileDescription: "Inx Launcher",
				internalName:    "inx-launcher",
				originalName:    "inx-launcher.exe",
			}
			...
			if got := (*info.Table()[version.LangDefault])[version.OriginalFilename]; got != "inx-launcher.exe" {
