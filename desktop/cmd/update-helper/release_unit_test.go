...
func TestLoadWindowsStagedReleaseUnitPreflightsAllMembersAndPublishesDesktopLast(t *testing.T) {
	...
	for name, content := range map[string]string{
		"inx-desktop.exe":       "desktop-v2",
		"inx-guard.exe":         "guard-v2",
		"inx-launcher.exe":      "launcher-v2",
		"inx-update-helper.exe": "helper-v2",
		"inx-cli.exe":           "cli-v2",
	} {
	...
	claimed := &repair.UpdateTransaction{
		...
		Files: []repair.UpdateTransactionFile{
			{TargetPath: filepath.Join(installDir, "inx-desktop.exe")},
			{TargetPath: filepath.Join(installDir, "inx-guard.exe")},
			{TargetPath: filepath.Join(installDir, "inx-launcher.exe")},
			{TargetPath: filepath.Join(installDir, "inx-update-helper.exe")},
			{TargetPath: filepath.Join(installDir, "inx-cli.exe")},
			{TargetPath: filepath.Join(installDir, "Inx.exe")},
		},
	}
	...
	if got := filepath.Base(members[len(members)-1].targetPath); !strings.EqualFold(got, "inx-desktop.exe") {
	...
	if got := strings.Join(published, ","); !strings.Contains(got, "Inx.exe=launcher-v2") {
		t.Fatalf("portable alias did not reuse launcher payload: %s", got)
	}
	if !strings.HasPrefix(published[len(published)-1], "inx-desktop.exe=") {
...
func TestLoadWindowsStagedReleaseUnitRejectsIncompletePayloadBeforePublish(t *testing.T) {
	staging := t.TempDir()
	if err := os.WriteFile(filepath.Join(staging, "inx-desktop.exe"), []byte("desktop-v2"), 0o700); err != nil {
		t.Fatal(err)
	}
	useTestWindowsPayloadManifest(t, staging, "v2")
	installDir := t.TempDir()
...
func TestValidateWindowsClaimedReleaseUnitRequiresExactTargets(t *testing.T) {
	...
	t.Run("primary", func(t *testing.T) {
		claimed := *complete
		claimed.TargetPath = filepath.Join(filepath.Dir(complete.TargetPath), "inx-guard.exe")
		if err := validateWindowsClaimedReleaseUnit(&claimed); err == nil ||
...
func TestLoadWindowsStagedReleaseUnitDoesNotCreateMissingPortableAlias(t *testing.T) {
	...
	claimed := &repair.UpdateTransaction{
		...
		Files: []repair.UpdateTransactionFile{
			...
			{TargetPath: filepath.Join(installDir, "inx-update-helper.exe")},
			{TargetPath: filepath.Join(installDir, "inx-cli.exe")},
			{TargetPath: filepath.Join(installDir, "Inx.exe"), MissingBefore: true},
		},
	}
	...
	for _, member := range members {
		if strings.EqualFold(filepath.Base(member.targetPath), "Inx.exe") {
...
func TestLoadWindowsStagedReleaseUnitRejectsManifestMemberHashDrift(t *testing.T) {
	...
	acceptWindowsPayloadManifestForTest(t)
	useUnverifiedPayloadReaderForTest(t)
	if err := os.WriteFile(filepath.Join(staging, "inx-guard.exe"), []byte("tampered"), 0o700); err != nil {
		t.Fatal(err)
	}
	if _, err := loadWindowsStagedReleaseUnit(claimed, staging); err == nil ||
