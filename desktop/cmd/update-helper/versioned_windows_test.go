...
func TestActivateVersionedWindowsFromStaging(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(installDir, "inx-desktop.exe"), []byte("old"), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(installDir, "inx-guard.exe"), []byte("old-guard"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || string(raw) != "payload:inx-desktop.exe" {
		t.Fatalf("desktop payload = %q err=%v", raw, err)
	}
	for _, name := range []string{"inx-launcher.exe", "Inx.exe", "inx-cli.exe"} {
		if _, err := os.Stat(filepath.Join(installDir, name)); err != nil {
			t.Fatalf("root entry %s: %v", name, err)
		}
	}
	if _, err := os.Stat(filepath.Join(installDir, "inx-desktop.exe")); !os.IsNotExist(err) {
		t.Fatal("flat desktop should be removed")
	}
	if _, err := os.Stat(filepath.Join(installDir, "inx-guard.exe")); !os.IsNotExist(err) {
		t.Fatal("flat guard should be removed")
	}
	if prefer := preferRelaunchPath("", installDir); filepath.Base(prefer) != "inx-launcher.exe" {
