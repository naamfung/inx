...
func TestCreateSubagentProfileScopeIsStrictButEmptyRemainsGlobal(t *testing.T) {
	...
	if !strings.Contains(filepath.ToSlash(path), "/.inx/skills/") {
		t.Fatalf("empty scope path = %q, want global Inx skills dir", path)
	}
...
func TestUpdateSubagentProfileRefusesNonManualSkill(t *testing.T) {
	...
	dir := filepath.Join(home, ".inx", "skills", "hand-authored")
...
func TestUpdateSubagentProfileRefusesUnmanagedFrontmatter(t *testing.T) {
	...
	dir := filepath.Join(home, ".inx", "skills", "manual-rich")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "SKILL.md"),
...
func TestUpdateSubagentProfileRefusesManualInlineSkill(t *testing.T) {
	a := newTestSubagentApp(t)
	home := os.Getenv("HOME")
	dir := filepath.Join(home, ".inx", "skills", "manual-inline")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "SKILL.md"),
...
func TestDeleteSubagentProfileRefusesNonProfileSkill(t *testing.T) {
	a := newTestSubagentApp(t)
	home := os.Getenv("HOME")
	dir := filepath.Join(home, ".inx", "skills", "hand-skill")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestUpdateSubagentProfileRefusesExpandedReferences(t *testing.T) {
	a := newTestSubagentApp(t)
	home := os.Getenv("HOME")
	dir := filepath.Join(home, ".inx", "skills", "with-refs")
...
func TestTrySubagentProfileCancelAbortsRunAndIsSingleFlight(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-t", Kind: "openai", BaseURL: srv.URL, Model: "model-t1", APIKeyEnv: "INX_TEST_KEY"},
	}
