...
func TestSingleInstanceIDScopesToInxHome(t *testing.T) {
	first := filepath.Join(t.TempDir(), "first")
	second := filepath.Join(t.TempDir(), "second")
	t.Setenv("INX_HOME", first)
	firstID := singleInstanceID()
	t.Setenv("INX_HOME", filepath.Join(first, "."))
	if got := singleInstanceID(); got != firstID {
		t.Fatalf("same data home produced different ids: %q != %q", got, firstID)
	}
	t.Setenv("INX_HOME", second)
	if got := singleInstanceID(); got == firstID {
...
func TestSingleInstanceIDResolvesMissingHomeThroughSymlink(t *testing.T) {
	...
	t.Setenv("INX_HOME", filepath.Join(realParent, "not-created", "home"))
	realID := singleInstanceID()
	t.Setenv("INX_HOME", filepath.Join(aliasParent, "not-created", "home"))
