...
func writeInxTestBundle(t *testing.T, path string) {
	t.Helper()
...
<plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.wails.inx-desktop</string></dict></plist>`
	if err := os.WriteFile(filepath.Join(path, "Contents", "Info.plist"), []byte(plist), 0o644); err != nil {
		t.Fatal(err)
	}
}

func TestRepairMacDesktopAliasesRetargetsBrokenInxAlias(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
	}
	oldApp := filepath.Join(root, "Inx.app.inx-update-backup")
	currentApp := filepath.Join(root, "Inx.app")
	writeInxTestBundle(t, oldApp)
	writeInxTestBundle(t, currentApp)
	alias := filepath.Join(desktop, "Inx")
...
func TestRepairMacDesktopAliasesPreservesUnrelatedBrokenAlias(t *testing.T) {
	...
	currentApp := filepath.Join(root, "Inx.app")
	writeInxTestBundle(t, oldTarget)
	writeInxTestBundle(t, currentApp)
...
func TestRepairMacDesktopAliasesPreservesOrdinaryInxFile(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
	...
	currentApp := filepath.Join(root, "Inx.app")
	writeInxTestBundle(t, currentApp)
	ordinary := filepath.Join(desktop, "Inx")
	...
	if string(got) != content {
		t.Fatalf("ordinary Inx file changed to %q", got)
	}
}

func TestRepairMacDesktopAliasesPreservesHealthyInxAlias(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
...
func TestRepairMacDesktopAliasesRetargetsCustomNamedInstalledAlias(t *testing.T) {
	...
	oldApp := filepath.Join(root, "Inx-old.app")
	currentApp := filepath.Join(root, "Inx.app")
	writeInxTestBundle(t, oldApp)
	writeInxTestBundle(t, currentApp)
	alias := filepath.Join(desktop, "My Inx")
	if err := writeMacAlias(oldApp, alias); err != nil {
		t.Fatal(err)
	}
...
func TestMacBundleOwnershipRejectsNonFileURL(t *testing.T) {
	owned, err := macURLReferencesInxBundle("https://example.com/Inx.app")
	if err != nil {
		t.Fatal(err)
	}
	if owned {
		t.Fatal("non-file URL was classified as a Inx application bundle")
	}
}

func TestMacBundleOwnershipRecognizesInstalledInxBundle(t *testing.T) {
	app := filepath.Join(t.TempDir(), "Inx.app")
	writeInxTestBundle(t, app)
	owned, err := macURLReferencesInxBundle((&url.URL{Scheme: "file", Path: app}).String())
	if err != nil {
		t.Fatal(err)
	}
	if !owned {
		t.Fatal("installed Inx application bundle was not recognized")
	}
}
