...
// remoteLegacyWorkbenchMirrorDir is the fixed Inx private directory that
...
// remoteLegacyWorkbenchTrustPath is the fixed Inx private file that held
...
// Only these two whitelisted targets inside the Inx private directory are
...
func (a *App) CleanRemoteLegacyWorkbenchData(target string) error {
	...
	case "mirrors":
		...
		if !withinInxPrivateDir(path) {
			return fmt.Errorf("legacy workbench mirrors path escapes the Inx data directory")
		}
	...
	case "trust":
		...
		if !withinInxPrivateDir(path) {
			return fmt.Errorf("legacy provider trust path escapes the Inx data directory")
		}
...
// withinInxPrivateDir verifies path resolves lexically inside the fixed
// Inx private state directory. MemoryUserDir is derived from the same
// env-scoped home the legacy artifacts were written into.
func withinInxPrivateDir(path string) bool {
