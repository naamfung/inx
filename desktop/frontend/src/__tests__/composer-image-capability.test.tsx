...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => {
      saveCalls += 1;
      return ".inx/attachments/mock.png";
    },
...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => ".inx/attachments/mock.png",
    AttachmentDataURL: async () => "data:image/png;base64,iVBORw0KGgo=",
  });
  ...
  ok(sent[0]?.submit?.includes("@.inx/attachments/mock.png") === true, "submitted text retains the local image attachment ref");
...

{
  ...
  const { root, paint } = renderUserMessage("check @[photo.png](.inx/attachments/mock.png)");
  await paint();
