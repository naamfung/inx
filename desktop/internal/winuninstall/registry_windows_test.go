...
func TestDeleteInstallLocationIfOwned(t *testing.T) {
	tests := []struct {
		...
		{name: "same root", location: `D:\Inx`, wantDelete: true},
		{name: "quoted case variant", location: `"d:\inx\"`, wantDelete: true},
		{name: "separate legacy install", location: `C:\Legacy\Inx`, wantDelete: false},
	}

	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			keyPath := fmt.Sprintf(`Software\InxTests\winuninstall-%d-%d`, os.Getpid(), i)
			...
			if err := deleteInstallLocationIfOwned(keyPath, `D:\Inx`); err != nil {
				t.Fatal(err)
			}
...
func TestDeleteInstallLocationIfOwnedIgnoresMissingKey(t *testing.T) {
	keyPath := fmt.Sprintf(`Software\InxTests\missing-%d`, os.Getpid())
	_ = registry.DeleteKey(registry.CURRENT_USER, keyPath)
	if err := deleteInstallLocationIfOwned(keyPath, `D:\Inx`); err != nil {
		t.Fatal(err)
	}
