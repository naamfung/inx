...
// installed Inx tree. Portable trees have no owned registration and no
...
func Plan(current, legacy *Registration, installRoot, version string, uninstallerPresent bool) (ReconcilePlan, error) {
	...
	return ReconcilePlan{
		...
		Desired: Registration{
			...
			DisplayIcon:          joinWindowsPath(root, "inx-launcher.exe"),
...
func registrationOwnsRoot(reg *Registration, root string) bool {
	if reg == nil || !strings.EqualFold(strings.TrimSpace(reg.DisplayName), "Inx") {
		return false
	}
